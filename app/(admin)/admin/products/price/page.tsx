...
export default function PriceImportPage() {
  ...
  const [preview, setPreview] = useState<PriceItem[]>([])
  ...
  function handleFile(file: File) {
    ...
    reader.onload = (e) => {
      ...
      const parsed: PriceItem[] = rows.map((row) => ({
        sku: String(row['SKU'] ?? row['sku'] ?? row['품목코드'] ?? '').trim(),
        price: Number(row['Price'] ?? row['price'] ?? row['가격'] ?? row['단가'] ?? 0),
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-8">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Update Price</h1>
      </div>
      ...
      <div
        ...
        className="border-2 border-dashed border-gray-300 rounded-xl p-12 text-center cursor-pointer hover:border-amber-400 hover:bg-amber-50 transition-colors mb-4"
      >
        ...
        {fileName && <p className="mt-2 text-amber-600 text-sm font-medium">{fileName}</p>}
        <input
          ref={fileRef}
          type="file"
          accept=".xlsx,.xls"
      ...
      <p className="text-xs text-gray-400 mb-6">
        Required columns: <span className="font-mono">SKU, Price</span>
        <br />
        Matches products by SKU and updates the unit price.
      </p>
      ...
      {preview.length > 0 && !result && (
        ...
        >
          {importing ? 'Updating...' : `Update ${preview.length} Prices`}
        </button>
      )}
      ...
      {result && (
        <div className="bg-white rounded-xl border p-4">
          <p className="text-sm font-medium text-gray-900 mb-1">Prices Updated</p>
